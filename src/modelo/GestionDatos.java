...
public class GestionDatos {
	...
	public RespuestaColocacion isMarcada(Coordenada coordenada) {
		...
		if (this.tablero.getCasilla(coordenada).isVelada()) {
			...
			respuesta = new RespuestaColocacion(true,
					String.valueOf(this.tablero.getCasilla(coordenada).getMinasAlrededor()), coordenada);
		}
		...
		if (this.tablero.getCasilla(coordenada).isMina()) {
			...
			for (int i = 0; i < this.tablero.getAlto(); i++) {
				for (int j = 0; j < this.tablero.getAncho(); j++) {
					...
					if (!casilla.isVelada()) {
						RespuestaColocacion respuesta = new RespuestaColocacion(true, String.valueOf(casilla.getMinasAlrededor()), auxiliar);
						arrayRespuestas.add(respuesta);
						if (casilla.getMinasAlrededor()==0&&casilla.isMina()) {
							respuesta.setMensaje("X");
						}
					}
				}
			}
